...
from dementia_prediction.transfer_learning.adni_finetuning import CNN
...
for key, value in patients_dict.items():
    if value == 0 and key in train_patients:
        train_dict_s[key] = value
    if value == 0 and key in valid_patients:
        valid_dict_s[key] = value
    if value == 1 and key in train_patients:
        train_dict_p[key] = value
    if value == 1 and key in valid_patients:
        valid_dict_p[key] = value
print("Total:", len(set(total_dict)))
'''
with open('dict_all.csv','w') as filep:
    csvwriter = csv.writer(filep)
    csvwriter.writerow(['Train',''])
    for i in train_dict_s.keys():
        csvwriter.writerow([i,0])
    for i in train_dict_p.keys():
        csvwriter.writerow([i,1])
    csvwriter.writerow(['Valid',''])
    for i in valid_dict_s.keys():
        csvwriter.writerow([i,0])
    for i in valid_dict_p.keys():
        csvwriter.writerow([i,1])
'''
