...
from dementia_prediction.transfer_learning.adni_t1_conv7 import CNN
...
for directory in os.walk(paths['datadir']):
    ...
    for file in directory[2]:
        ...
        regex = r"-T1_brain_mni_aligned.nii\.gz$"
        if re.search(regex, input_file):
            #pat_code = input_file.rsplit('_normalized.nii.gz')
            pat_code = input_file.rsplit('-T1_brain_mni_aligned.nii.gz')
            patient_code = pat_code[0].rsplit('/', 1)[1]
