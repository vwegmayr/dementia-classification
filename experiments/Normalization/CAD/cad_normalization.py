...
class Normalize():
    ...
    def __init__(self, mode):
        ...
        self.mean_path = paths['norm_mean_var'] + \
                         'adni/adni_train_data_mean.pkl'
        self.var_path = paths['norm_mean_var'] + \
                        'adni/adni_train_data_var.pkl'
        self.mean_norm = []
        self.var_norm = []
    ...
    def get_files(self, dir):
        ...
        for directory in os.walk(dir):
            ...
            for file in directory[2]:
                ...
                regex = r"_bet_mni_aligned\.nii\.gz$"
    ...
    def store(self, data):
        for filename in data:
                pat_code = filename.rsplit('_bet_mni_aligned.nii.gz')
...
def main():
    norm_object = Normalize('cad')
    train_patients = norm_object.get_files(paths['cad_path'])
    print("Train: ", len(train_patients))
    num_parallel = 20 
    pool = ProcessPool(num_parallel)
    all_patients = train_patients # per-image normalization of all images
    ...
    train_patients = norm_object.get_files(paths['adni_per_image_out'])
    all_patients = train_patients 
