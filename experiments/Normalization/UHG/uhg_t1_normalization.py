...
filep = open(paths['uhg_valid_path'], 'rb')
valid_dict = pickle.load(filep)
print("Valid", len(valid_dict), "Patients")

filep = open(paths['uhg_train_path'], 'rb')
...
class Normalize():
        ...
        for directory in os.walk(dir):
            ...
            for file in directory[2]:
                ...
                if re.search(regex, input_file):
                    pat_code = input_file.\
                                rsplit('-T1_brain_mni_aligned.nii.gz')
    ...
    def store(self, data):
        for filename in data:
                pat_code = filename.rsplit('-T1_brain_mni_aligned.nii.gz')
                patient_code = pat_code[0].rsplit('/', 1)[1]
                output = paths['norm_out']+patient_code+'_normalized.nii.gz'
...
def main():
    ...
    print("Retrieving files from ", paths['per_image_out'])
    sys.stdout.flush()
    train_patients, valid_patients = norm_object.get_files(paths['per_image_out'])
