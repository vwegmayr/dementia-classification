...
class CNN:
    ...
    def inference(self, images, keep_prob, is_training):
        ...
        with tf.variable_scope('conv7') as scope:
            conv_last = self.conv_relu(input_=conv6,
                                   kernel_shape=[3, 3, 3, 256, 512],
                                   biases_shape=[512],
                                   decay_constant=self.param['decay_const'],
                                   scope=scope,padding='SAME',
                                   stride=2, is_training=is_training, keep_prob=keep_prob)
        print("Conv7", conv_last.get_shape())
        """
        ...
        with tf.variable_scope('fullcn2') as scope:
            vector_per_batch = tf.reshape(conv_last, [self.param['batch_size'],
                                          -1])
            ...
            relufcn = tf.nn.relu(pre_activation2, name=scope.name)
            fullcn_drop = tf.nn.dropout(relufcn, keep_prob)
            print(fullcn_drop.get_shape())
            tf.add_to_collection('restorevar', fullcn_drop)
    ...
    def evaluation(self, sess, eval_op, dataset, images, labels, logits, keep_prob,
        ...
        for step in range(num_steps):
            ...
            predictions, correct_, logits_, loss_ = sess.run([eval_op, corr, logits, loss],
                feed_dict={
                    images: image_data,
                    labels: label_data,
                    keep_prob: 1.0,
