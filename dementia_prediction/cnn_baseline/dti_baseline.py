...
class CNN:
    ...
    def conv_relu(self, input_, kernel_shape, biases_shape, decay_constant,
        ...
        bn = tf.contrib.layers.batch_norm(act_relu,
    ...
    def inference(self, images, keep_prob, is_training):
        ...
        with tf.variable_scope('conv1_a') as scope:
            bn = tf.contrib.layers.batch_norm(images,
                                              center=True, scale=True,
                                              is_training=is_training)
            conv1_a = self.conv_relu(input_=bn,
        ...
        with tf.variable_scope('conv1_b') as scope:
            conv1_b = self.conv_relu(input_=bn,
        ...
        with tf.variable_scope('conv1_c') as scope:
            conv1_c = self.conv_relu(input_=bn,
        ...
        with tf.variable_scope('fullcn2') as scope:
            ...
            fullcn = tf.nn.dropout(pre_activation2, keep_prob)
            bn2 = tf.contrib.layers.batch_norm(ctivation2,
                                              center=True, scale=True,
                                              is_training=is_training)
            """
        ...
        with tf.variable_scope('logits') as scope:
            ...
            biases = self.variable_on_cpu(name='biases',
                                          ...
                                          initializer=tf.constant_initializer(
                                              0.1))
            logits = tf.add(tf.matmul(fullcn, weights), biases,
                            name=scope.name)
